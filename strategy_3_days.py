...
Strategy:
  ...
  4. Post a new lending offer at benchmark_apy * 0.95 (5% below benchmark) to be
     competitive — a shallower undercut than 7/15-day since this strategy's higher
     LTV floor already compensates for its risk, without also pricing above market.
  5. Enforce:
...
# Strategy parameters
APY_DISCOUNT = 0.05          # undercut the market benchmark (volume-weighted median) by 5% —
                              # shallower than 7/15-day since the risk premium for this
                              # strategy's higher LTV floor is already handled by the LTV
                              # mechanism itself, not by pricing above the market
MAX_DURATION_DAYS = 3
...
# the target.
FALLBACK_LTV = 0.60                        # used when there's not enough market data for the token
