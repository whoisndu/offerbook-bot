...
- **[`strategy/`](strategy/README.md)** — scripts that place, fill, or cancel live offers: `strategy.py` (the main bot — see its README for the full pricing/risk math), `fixed_terms_offers.py` (fixed-LTV/fixed-APY multi-duration offers, sized off available balance rather than market-benchmarked), `defaulter_capture.py`, `create_targeted_offers.py` (gitignored), `cancel_offers.py`, `fill_offer.py`, `update_config.py`. Also holds `allocation_config.yaml` (gitignored).
...
- **[`reporting/`](reporting/README.md)** — read-only analytics, never signs anything: `pnl_leaderboard.py`, `lender_capital_scan.py`, `competitor_timing_report.py`, `offer_posting_times.py`, `borrower_loan_timeline.py`, `address_snapshot.py` (fast chart-free lookup, one or more addresses combined), `liquidity_check.py` (real available liquidity for a pair, correcting for rehypothecation), `verify_offers.py`, `portfolio_health.py`.
